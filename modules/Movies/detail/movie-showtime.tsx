...
export default function MovieShowtimeDetail({ movieId }: { movieId: number }) {
  ...
  const selectedDay = showtimeData?.full_showtimes?.find(
  ...
  const groupedShowtimes = selectedDay
    ? selectedDay.showtimes.reduce((groups: any, show: any) => {
  ...
  return (
    <div className="mt-6">
      ...
      <div className="flex gap-4 mb-4 border-b pb-2">
        {showtimeData.full_showtimes.map((day: any) => (
          <button
            key={day.date}
